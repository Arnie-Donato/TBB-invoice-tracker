...
Deno.serve(async (request) => {
  ...
  if (!code || !state) return Response.redirect(`${appUrl}?drive=failed`, 302)
  const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!)
  const { data: pending } = await supabase.from('drive_oauth_states').select('*').eq('state', state).gt('expires_at', new Date().toISOString()).maybeSingle()
  if (!pending) return Response.redirect(`${appUrl}?drive=expired`, 302)
